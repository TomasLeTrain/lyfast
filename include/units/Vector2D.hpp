...
class Vector2D {
  public:
    ...
    /**
     * @brief Constructs vector from another of the same quantity and
     * same/different floatType
     *
     * @param other Other vector
     */
    template<sameQuantity<T> Q>
    constexpr Vector2D(Vector2D<Q> other)
        : x(other.x),
          y(other.y) {}
     ...
     *
     * This constructor takes polar coordinates and converts them to cartesian
     * coordinates
     *
     * @param t angle
     * @param m magnitude
     */
    static constexpr Vector2D fromPolar(Angle t, T m) {
        m = abs(m);
     ...
     * @return Vector2D
     */
    static constexpr Vector2D unitVector(Angle t) {
        return fromPolar(t, (T)1.0);
    }
    ...
    /**
     * @brief - operator overload. Substracts the x and y components of two
     * vectors
     *
     * {a, b} - {c, d} = {a - c, b - d}
     *
     ...
     * @return Vector2D<T>
     */
    constexpr Vector2D<T> operator*(T::floatType factor) const {
        return Vector2D<T>(this->x * factor, this->y * factor);
    }
    ...
    constexpr Vector2D<R> operator*(Q factor) const {
     ...
     */
    template<isQuantity Q, isQuantity R = Multiplied<T, Q>>
    constexpr R operator*(const Vector2D<Q>& other) const {
     ...
     * @return Vector2D<T>
     */
    template<isQuantity Q, isQuantity R = Divided<T, Q>>
    constexpr Vector2D<R> operator/(Q factor) const {
        return Vector2D<R>(this->x / factor, this->y / factor);
    }

    /**
     * @brief += operator overload. Adds the components of two vectors and
     * stores the result
     *
     * {a, b} += {c, d} => {a + c, b + d}
     *
    ...
    /**
     * @brief -= operator overload. Subtracts the components of two vectors and
     * stores the result
     *
     * {a, b} -= {c, d} => {a - c, b - d}
     *
    ...
    /**
     * @brief *= operator overload. Multiplies the components of a vector by a
     ...
     * @return Vector2D<T>&
     */
    constexpr Vector2D<T>& operator*=(T::floatType factor) {
        this->x *= factor;
        this->y *= factor;
        return (*this);
    }
    ...
    /**
     * @brief /= operator overload. Divides the components of a vector by a
     * scalar and stores the result
     *
     * {a, b} /= c => {a / c, b / c}
     *
     ...
     * @return Vector2D<T>
     */
    constexpr Vector2D<Number> normalize() const {
...
template<isQuantity T>
inline constexpr Vector2D<T> origin({ T(0.0), T(0.0) });
 ...
 */
template<isQuantity Q>
constexpr Vector2D<Q> operator*(typename Q::floatType lhs,
                                const Vector2D<Q>& rhs) {
    return rhs * lhs;
}
...
// define some common vector types
typedef Vector2D<FLength> V2FPosition;
typedef Vector2D<FLinearVelocity> V2FVelocity;
typedef Vector2D<FLinearAcceleration> V2FAcceleration;
