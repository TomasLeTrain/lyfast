...
inline void printPairAsLatex(std::pair<float, float> data) {
...
inline void
printPairListAsLatex(size_t size,
...
inline void printListAsLatex(size_t size,
                             std::function<float(size_t)> num_func) {
    std::cout << "\\left[";
    for (size_t i = 0; i < size; i++) {
        std::cout << num_func(i);
